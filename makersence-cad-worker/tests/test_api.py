...
def test_legacy_generate_contract_requires_explicit_strategy():
    payload={
      'module':'system_selftest','appearance_hash':'abc','idempotency_key':'test-legacy-explicit-1',
      'cad_contract':{
        'generation_strategy':'LEGACY_CAD',
        'family':'silhouette_plate','width_mm':60,'height_mm':38,'thickness_mm':2.4,'wall_mm':2.4,
        'separate_parts':[{'name':'TEST_FRAME','role':'photo_frame','kind':'rect_frame','outer_width_mm':20,'outer_height_mm':20,'inner_width_mm':16,'inner_height_mm':16,'thickness_mm':1.2,'print_x_mm':42}]
      },
    ...
    job=_wait(r.json()['job_id'],40)
    assert job['status']=='completed',job
    assert job['adapter']=='LEGACY'
    assert job['validation']['brep_valid'] is True
    assert {'stl','step','3mf','glb','png','json'} <= {a['type'] for a in job['artifacts']}

def test_missing_strategy_fails_closed_instead_of_plate_fallback():
    payload={
      'module':'unknown_future_product',
      'idempotency_key':'test-no-silent-legacy-1',
      'cad_contract':{'width_mm':60,'height_mm':38,'thickness_mm':2.4}
    }
    r=client.post('/v1/generate',json=payload,headers=H);assert r.status_code==200
    job=_wait(r.json()['job_id'],40)
    assert job['status']=='failed',job
    assert 'explicit CAD generation_strategy/adapter is required' in (job.get('error') or '')
